...
Подключён **Xaero's Minimap** через переменную `MODS` в `compose.yaml` — образ скачивает мод по URL при старте контейнера.

Добавить моды можно двумя способами:

1. **Через `MODS` в compose** — укажите URL jar-файла в переменной окружения `MODS` (в `compose.yaml`). При деплое или перезапуске контейнер загрузит моды автоматически. Для нескольких модов используйте многострочное значение с `|`.
2. **Вручную** — положите `.jar` в `./data/mods/` на сервере и выполните `docker compose restart`.

Чтобы мини-карта работала у игроков, на клиенте тоже должен быть установлен тот же мод (Forge 1.20.1).
